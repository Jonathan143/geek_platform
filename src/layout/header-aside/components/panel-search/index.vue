<template>
  <div class="panel-search"
    flex="dir:top">
    <div class="panel-search__input-group"
      flex-box="0"
      flex="dir:top main:center cross:center"
      @click.self="handlePanelClick">
      <d2-icon-svg class="panel-search__logo"
        name="d2-admin-text" />
      <el-autocomplete class="panel-search__input"
        ref="input"
        v-model="searchText"
        ...
        @select="handleSelect">
        <d2-panel-search-item slot-scope="{ item }"
          :item="item" />
      </el-autocomplete>
    ...
    <div v-if="resultsList.length > 0"
      ...
      <el-card>
        <div class="panel-search__results-group-inner">
          <d2-panel-search-item v-for="(item, index) in resultsList"
            :key="index"
            :item="item"
            :hover-mode="true"
            @click.native="handleResultsGroupItemClick(item.route)" />
        </div>
      </el-card>
    </div>
  </div>
...
export default {
  ...
  methods: {
     ...
     */
    handleResultsGroupItemClick({ name }) {
    ...
    handleSelect({ route }) {
      ...
      // 用户选择的是其它页面
      this.$nextTick(() => {
        this.handleMenuSelect(route.name)
      })
    },
    /**
     * @augments 关闭输入框的下拉菜单
